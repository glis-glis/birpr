...
* Use a negative-binomial model when your data is overdispersed
* Add covariates that account for survey effort or detection probability to improve model accuracy
* Understand the modelling approach behind ```birp``` and how it works
...
If your study has no intervention, like in this example, you can assign the same group name (e.g.,  ```Group_1 ```) to all rows. Each data set should correspond to a single survey method. If your study used multiple survey methods, ```birp``` can analyse all methods together; [this will be covered in a later section](#Combine data from different survey methods).
...
Once you prepared your data set, the next step is to convert it into a ```birp_data``` object, which is the required format for analysis using the ```birp``` package. There are three main functions available for creating a ```birp_data``` object, depending on the format and source of your input data:
...
Let's convert the example data from above to a ```birp_data``` object. Since our example data is an already existing data frame in R, we will use the ```birp_data_from_data_frame()``` function:
```{r}
exampleBirp <- birp_data_from_data_frame(exampleData)
print(exampleBirp)
```

When printing the object, you can see a summary of your data, and you can always access the original data frame again using the dollar sign with your ```birp_data``` object: ```exampleBirp$data```.
...
After fitting a model with ```birp()```, you can easily visualize the estimated trends using the ```plot()``` function. This works because ```birp``` objects have a custom plot method defined for them, so when you use ```plot()``` with a ```birp``` object, it automatically uses the underlying function tailored to show the posterior distributions of the trend parameter ($\gamma$). 
```{r, fig.width=6, fig.height=4}
plot(est)
```

The x-axis represents the rate parameter $\gamma$. A positive $\gamma$ indicates an increasing population trend, while a negative $\gamma$ indicates a decreasing trend. The legend in the top corner is added automatically and shows the posterior probability that the population is increasing, based on the observed data (i.e., the number of counts, $n$).
...
############################################
# Combine data from different survey methods
###########################################
...
res_assess <- assess_NB(est, numRep = 5, verbose=FALSE)
```
