...
###############################################################
## Before-After (BA) Design with Step Changes
###############################################################
...
If you believe the event caused a sudden shift in abundance but no change in the underlying trend, set change = "step". In this case, there is no need to define a ```rate_design```, as we are not estimating any rates:

```{r, echo=FALSE}
step_design      <- matrix(c("Group_1", 1,0), nrow = 1)
est_step_only <- birp(exampleBirp,
                      verbose = FALSE,
                      timesOfChange = 2023,
                      step_design = step_design,
                      change = "step")
