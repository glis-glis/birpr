...
This is the **main result of interest**. In this example, ```birp``` estimates a **`r round(est$post_gamma$prob_positive,4)*100`% probability that the trend is increasing** between the specified timepoints (e.g., 2020-2024). In other words, based on the observed data and the assumptions of the model, there's a **high posterior probability that the underlying population is growing** during this period. (Keep in mind that the underlying algorithm is stochastic, so your result when executing the command above may deviate slighlty from what is shown here)
...
shows **moderate support for an increasing trend before 2023**, and **stronger evidence for an increase after 2023**. You can test for multiple change points by providing a vector to the ```timesOfChange``` argument.
